...
class Vector3(object):
    ...
    def getCamVals(self,data):
        ...
        if camDistance >= 0 or True:
            return (
                data.width/2-tempVect.dot(data.camera.xVector)/camDistance*1000,
                data.height/2+tempVect.dot(data.camera.yVector)/camDistance*1000
            )
        return None

    def getDistance(self,pos):
        return ((self.x-pos.x)**2+(self.y-pos.y)**2+(self.z-pos.z)**2)**0.5
...
class Camera(object):
    ...
    def setPos(self,vector):
        if type(vector) == Vector3: self.position = vector
    ...
    def updateRenderList(self,data):
        ...
        else: self.renderList = sorted(self.renderList, reverse=True,
                key=lambda item: getAverageDistanceToCamera(data, item))
...
def openFile(data):
    ...
    if tempDir == '' or tempDir[-4:-1]+tempDir[-1] != '.obj': return False
    ...
    for v in data.vertex: v.setY(v.getY()-mini)
    ...
    maxi = 0
    for v in data.vertex: maxi = max(v.getSize(),maxi)
...
def saveFile(data):
    ...
    if len(data.openFileLines) < 1: return
...
def initBotFrame(data):
    ...
    data.gridCheck = Checkbutton(data.botLeftFrame, text="GRID",
                            ...
                            onvalue=True, offvalue=False, fg="white", bg="gray",
                            font="msserif 10 bold", selectcolor="gray20",
                            command=lambda: updateDraw(data))
...
def vertexEntryCheck(data,box,event):
    ...
    if data.curSelect == None: entryVar.set(0)
    else:entryVar.set(data.vertex[data.curSelect].__dict__[box.lower()])
...
def init(data):
    ...
    for i in range(20):
        for j in range(20):
            data.gridDots.append([Vector3(i-10,0,j-10), Vector3(i-10,0,j-9),
...
def mousePressed(event, data):
    ...
    if data.tempSelect != None:
        if data.tempSelect == data.curSelect: deselectVertex(data)
        else: selectVertex(data,data.tempSelect)
        data.drawUpdate = True
...
def timerFired(data):
    ...
    if data.mouseMoved:
        ...
        for i in range(len(data.camera.renderVertex)):
            ...
            if dist < mini: mini,minIndex = dist,i
        if mini < 6: data.tempSelect = minIndex
        else: data.tempSelect = None
        data.mouseMoved = False
...
def redrawAll(canvas, data):
    ...
    for face in data.camera.renderList:
        if getAverageDistanceToCamera(data,face) > 0:
            canvas.create_polygon([data.camera.renderVertex[i-1] for i in face],
